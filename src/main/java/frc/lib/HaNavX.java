...
public class HaNavX implements Sendable {
	...
	public HaNavX(SerialPort.Port port) {
		try {
			...
			DriverStation.reportWarning("Failed to connect to navX.", false);
		}
	}
	...
	public void zeroYaw() {
		...
		try {
			...
			DriverStation.reportWarning("Failed to zero navX yaw.", false);
		}
	}
	...
	public void setYaw(double offsetDeg) {
		...
		DriverStation.reportWarning("Gyro set to: " + Double.toString(offsetDeg) + " degrees.", false);
	}
	...
	private void initReport() {
		...
		if (this.commsTimoutTimer.hasElapsed(kTimeoutSec)) {
			...
			DriverStation.reportWarning("navX done calibrating in " + Double.toString(this.commsTimoutTimer.get())
					+ " seconds from startup.", false);
		}
